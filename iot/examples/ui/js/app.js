...
function onTurnLigthsTopic(data) {
	var msj = 'The lights are ' + data.status + ' because its intensity is: ' + data.intensity;
	onLedTopic(data);
	putHouseMessage(msj);
};
function onSonarTopic(data) { putHouseMessage('WARNING: something detected. Distance: ' + data.distance + ' cm'); };
...
function onDoorTopic(data) { 
	...
	$('#door_txt').text( data.status === 'off' ? ' CLOSED' : ' OPEN' );
};
function onTempTopic(data) { $('#txt_temp').text(data.temperature.concat(' °C')); };
