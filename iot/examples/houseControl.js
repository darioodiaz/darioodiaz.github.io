...
function initSensors() {
	...
	//let tempSensor = new j5.Thermometer({ controller: 'LM35', pin: 'A1' });
...
function sendTempData() { client.publish('/temp', { temperature: this.celsius }); };
...
function sendLightData() { 
	...
	if (intensity < 100 && !ligthNotified) {
		redLed.on();
		client.publish('/turnLights', { intensity, status: 'on' });
		ligthNotified = true;
	} else if (intensity <= 100 && ligthNotified) {
		redLed.off();
